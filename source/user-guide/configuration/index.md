...
## LanceDB SQL Reference

Complete SQL syntax reference for LanceDB operations.

### DDL Operations

#### CREATE SCHEMA

Creates a new namespace (schema) with optional warehouse path:
...
Drops a namespace and all its contents:
...
Creates a table with optional vector column configuration:
...
-- Table with fixed-size list embedding
CREATE TABLE catalog.schema.articles (
    id BIGINT NOT NULL,
    title VARCHAR NOT NULL,
    embedding ARRAY<FLOAT>[768] NOT NULL
) WITH (
    'vector_column' = 'embedding',
...
-- Create IVF_FLAT index (highest accuracy)
CREATE INDEX idx_embedding ON catalog.schema.table_name
USING IVF_FLAT (embedding);

-- Create IVF_HNSW_SQ index (fast search, moderate memory)
CREATE INDEX idx_embedding ON catalog.schema.table_name
USING IVF_HNSW_SQ (embedding);

-- Create IVF_HNSW_PQ index (fast search, low memory)
CREATE INDEX idx_embedding ON catalog.schema.table_name
USING IVF_HNSW_PQ (embedding);

-- Create FLAT index (brute force, exact match)
CREATE INDEX idx_embedding ON catalog.schema.table_name
USING FLAT (embedding);

-- Create index only if not exists
CREATE INDEX IF NOT EXISTS idx_embedding ON catalog.schema.table_name
USING IVF_PQ (embedding);
```
...
INSERT INTO catalog.schema.products
SELECT
    id,
    UPPER(name) as name,
    price * 1.1 as price,
...
Standard SQL SELECT with full DataFusion SQL support:
...
SELECT
    id, name,
    dot_product(embedding, [0.1, 0.2, 0.3, ...]) as score
FROM products
ORDER BY score DESC
LIMIT 10;
```
...
PATHSDATA supports pgvector-compatible operators for vector operations:

```sql
-- <-> operator: L2 distance (lower is better)
SELECT id, name, embedding <-> [0.1, 0.2, 0.3, ...] as distance
FROM products
ORDER BY distance ASC
LIMIT 10;

-- <=> operator: Cosine distance (lower is better)
SELECT id, name, embedding <=> [0.1, 0.2, 0.3, ...] as distance
FROM products
ORDER BY distance ASC
LIMIT 10;

-- <#> operator: Negative inner product (lower is better)
...
-- BM25 score from FTS index (actual relevance ranking)
SELECT
    id, title,
    _fts_score as bm25_score
FROM articles
WHERE match_text(content, 'machine learning')
ORDER BY _fts_score DESC
LIMIT 10;
```
...
SELECT
    ...
    0.5 * (1.0 - match_text_score(content, 'query')) as combined
FROM articles
WHERE match_text(content, 'query')
